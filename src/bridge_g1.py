...
SNN_SRC = _REPO_ROOT / "deploy"
_RUNTIME = Path(tempfile.gettempdir()) / "fssc"
_RUNTIME.mkdir(exist_ok=True)
TELEMETRY_PATH = str(_RUNTIME / "bridge_telemetry.json")
STATE_TSV_PATH = str(_RUNTIME / "bridge_state.tsv")
    ...
    def __init__(self, duration_sec: float = 14.0, cmd_vx: float = 0.8,
        ...
        if scene is not None:
            raise NotImplementedError(
                "Terrain scenes are not shipped in this release; run the "
                "fault battery on the flat scene (scene=None).")
        self.dep = Deploy12()
